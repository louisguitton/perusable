...
class ViewTests(APITestCase):
    ...
    def test_empty_query_returns_everything(self):
        ...
        self.assertJSONEqual(
            response.content,
            WineSerializer(wines, many=True).data,
        )
...
class ESViewTests(APITestCase):
    ...
    def test_description_highlights_matched_words(self):
        ...
        self.assertEquals(
            "A delicious bottle of <mark>wine</mark>.", response.data[0]["description"]
        )

    def test_suggests_words_for_spelling_mistakes(self):
        response = self.client.get(
            "/api/v1/catalog/es-wine-search-words/?query=cabernay"
        )
        self.assertEqual(1, len(response.data))
        self.assertEqual("cabernet", response.data[0]["word"])
