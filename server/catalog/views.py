    ...
    def get(self, request, *args, **kwargs):
        ...
        if query:
            ...
            search = search.highlight_options(  # new
                number_of_fragments=0, pre_tags=["<mark>"], post_tags=["</mark>"]
            )
                ...
                data=[
                    {
                        ...
                        "variety": (
                            ...
                            if "highlight" in hit.meta
                            and "variety" in hit.meta.highlight
                            else hit.variety
                        ),
                        "winery": (
                            hit.meta.highlight.winery[0]
                            if "highlight" in hit.meta
                            and "winery" in hit.meta.highlight
                            else hit.winery
                        ),
                    }
                    for hit in response
...
class ESWineSearchWordsView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        # Build Elasticsearch query.
        search = Search(index=constants.ES_INDEX).suggest(
            "result", query, term={"field": "all_text"}
        )
